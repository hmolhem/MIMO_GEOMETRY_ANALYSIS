...
class Z6ArrayProcessor(BaseArrayProcessor):
    """
    Implements the Weight-Constrained Sparse Array Z6.
    Z6 Array: 5-Sparse ULA of (N-4) sensors + 4 Augmented Sensors.
    ...
    def __init__(self, N: int, d: int = 1):
        ...
        # 1. Generate the (N-4)-sensor 5-sparse ULA segment
        # Positions: [0, 5d, 10d, ..., 5(N-5)d]
        N_sparse = N - 4
        sparse_segment = np.arange(N_sparse) * 5 * d
        
        # 2. Augmented sensors based on the paper's description (Sec. IV-D)
        # S0: -7d, S1: -4d, S(N-2): (5N-22)d, S(N-1): (5N-19)d
        augmented_sensors = np.array([-7 * d, -4 * d, (5 * N - 22) * d, (5 * N - 19) * d])
        
        # Combine all sensors
        positions = np.unique(np.concatenate((augmented_sensors, sparse_segment)))
        
        # Normalize the array so the smallest position is 0 (shift by +7d)
    ...
    def plot_coarray(self):
        ...
        print("\n[Plotting Placeholder]: Array Z6 Visualization (w(1)=w(2)=0)")
    ...
    def analyze_contiguous_segments(self):
        ...
        # ULA segment starts at L1=3 and ends at L2=5N-21[cite: 380]. Length L=5N-23.
        L1 = 3
        L2 = 5 * self.N_total - 21
        L = L2 - L1 + 1
