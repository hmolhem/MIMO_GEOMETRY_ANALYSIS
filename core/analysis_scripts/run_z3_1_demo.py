...
from core.radarpy.geometry.z3_1_processor import Z3_1ArrayProcessor
...
def main():
    parser = argparse.ArgumentParser(description="Run Z3(1) array geometry/coarray analysis.")
    ...
    parser.add_argument("--assert", dest="do_asserts", action="store_true",
                        help="Fail if Z3(1) invariants are violated.")
    ...
    print(f"--- Starting Array Z3(1) (N={args.N}, d={args.d}) Analysis Demo ---")

    z3 = Z3_1ArrayProcessor(N=args.N, d=args.d)
    ...
    # Performance summary
    print("\n" + "="*40)
    print("      ARRAY Z3(1) PERFORMANCE SUMMARY")
    print("="*40)
    ...
    # ------- Optional invariants for Z3(1) (for N >= 5) -------
    # From your notes: L1=2, L2=4N-13  => L = 4N-14
    # A = 4N-9; holes at {1, A-3, A-1}; and weights: w(1)=0, w(2)=2, w(3)=1
    N = args.N
    A = 4 * N - 9
    expected_L  = 4 * N - 14
    expected_w  = {1: 0, 2: 2, 3: 1}
    ...
    # extract weights at lags 1..3 (safe if missing)
    ...
    if args.do_asserts:
        ...
        assert expected_holes == holes, f"Holes mismatch: expected {expected_holes}, got {holes}"
    ...
    if args.save:
        ...
        out_path = os.path.join(args.save, f"z3_1_summary_N{args.N}_d{args.d}.csv")
        try:
