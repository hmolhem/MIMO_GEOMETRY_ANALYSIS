"""
Test Suite for CADiS Array Processor
=====================================

Tests CADiS (Concatenated Difference Set) array implementation.
...
def test_cadis_construction():
    """
    TEST 1: CADiS Array Construction
    
    Verify basic construction for simple case (N=5).
    Expected: Positions with w(1)=0 (no consecutive sensors)
    """
    print("="*80)
    print("TEST 1: CADiS Construction")
    print("="*80)
    
    try:
        processor = CADiSArrayProcessor(N=5, d=1.0)
...
def test_cadis_analysis():
    ...
    try:
        processor = CADiSArrayProcessor(N=8, d=1.0)
...
def test_weight_w1_zero():
    ...
    Verify CADiS achieves w(1)=0 constraint (critical property).
    This is the defining property of CADiS arrays (like Z5, MISC).
    """
    print("\n" + "="*80)
    ...
    try:
        ...
        assert w1 == 0, f"CADiS requires w(1)=0, got w(1)={w1}"
...
def test_cadis_vs_misc_comparison():
    """
    TEST 5: CADiS vs MISC Comparison
    
    Compare CADiS with MISC for N=7.
    Both should have w(1)=0 but may differ in aperture and L.
    """
    print("\n" + "="*80)
    print("TEST 5: CADiS vs MISC Comparison")
    print("="*80)
    ...
    try:
        ...
        # Run both processors
        cadis_proc = CADiSArrayProcessor(N=7, d=1.0)
        misc_proc = MISCArrayProcessor(N=7, d=1.0)
        
        cadis_results = cadis_proc.run_full_analysis()
        misc_results = misc_proc.run_full_analysis()
        
        # Extract weights
        cadis_wt = dict(zip(cadis_results.weight_table['Lag'], 
                           cadis_results.weight_table['Weight']))
        misc_wt = dict(zip(misc_results.weight_table['Lag'], 
                          misc_results.weight_table['Weight']))
        
        # Both should have w(1)=0
        assert cadis_wt.get(1, 0) == 0, "CADiS w(1) != 0"
        assert misc_wt.get(1, 0) == 0, "MISC w(1) != 0"
        
        print(f"   CADiS: A={cadis_results.coarray_aperture}, L={cadis_results.segment_length}")
        print(f"   MISC:  A={misc_results.coarray_aperture}, L={misc_results.segment_length}")
        print(f"✅ Both arrays achieve w(1)=0")
        
    except ImportError:
        print(f"⚠️  MISC processor not available - skipping comparison")
    except AssertionError as e:
        print(f"❌ TEST FAILED: {e}")
    except Exception as e:
        print(f"❌ TEST ERROR: {e}")
...
def test_robustness():
    ...
    Test CADiS construction and analysis across multiple N values.
    ...
    for N, d in test_configs:
        try:
            processor = CADiSArrayProcessor(N=N, d=d)
            results = processor.run_full_analysis()
...
def run_all_tests():
    """
    Run all CADiS array tests.
    """
    print("\n" + "="*80)
    print("  CADiS ARRAY PROCESSOR TEST SUITE")
    print("="*80 + "\n")
    
    tests = [
        ("Construction", test_cadis_construction),
        ("Analysis Pipeline", test_cadis_analysis),
        ("Weight w(1)=0", test_weight_w1_zero),
        ("N=7 Properties", test_cadis_n7_properties),
        ("CADiS vs MISC", test_cadis_vs_misc_comparison),
    ...
    if passed == total:
        print("\n🎉 ALL TESTS PASSED! CADiS implementation validated.")
    else:
