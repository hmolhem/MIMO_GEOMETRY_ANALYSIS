"""
Package-level runner utilities (pilot refactor of run_benchmarks)

This module provides a callable function `run_benchmarks(argv=None)` that
implements the main logic previously embedded in the script
`core/analysis_scripts/run_benchmarks.py`.

The module keeps behavior identical but is importable for tests and for
incremental refactors.
"""
from pathlib import Path
import sys

# Ensure repository root is on sys.path (mimic original script behavior)
...
from core.radarpy.signal.doa_sim_core import run_music, music_spectrum
from core.radarpy.signal.array_manifold import steering_vector
...
def simulate_snapshots(positions, d_phys, wavelength, doas_deg, snapshots, snr_db, seed=None):
    ...
    A = _steering_matrix(positions, d_phys, wavelength, doas_deg)
...
def run_alg(alg_name, X, positions, d_phys, wavelength, K, scan_deg=(-60, 60, 0.1), use_root=False,
    ...
    if alg_name == "SpatialMUSIC":
        doas_est, info = estimate_doa_spatial_music(X, positions, d_phys, wavelength, K, scan_deg=scan_deg)
        dbg = {"Mv": len(positions)}
        dbg.update(info)
        return doas_est, dbg
        ...
        dbg = {"Mv": int(dbg_full.get("Mv", 1))}
        if "Rv_singular" in dbg_full:
            dbg["Rv_singular"] = dbg_full["Rv_singular"]
            dbg["Rv_cond"] = dbg_full["Rv_cond"]
        return doas_est, dbg
    else:
        raise ValueError(f"Unknown algorithm: {alg_name}")


def run_benchmarks(argv=None):
    """Run benchmark job. `argv` may be a list of args or None to use sys.argv.

    Returns: path to saved CSV (args.out)
    """
    ...
    ap.add_argument("--delta", type=str, default="2")
    ...
    ap.add_argument("--dump-svd", action="store_true")
    ...
    for arr in arrays:
        ...
        kd_ula = float(k * args.d)
        kd_max = float(k * max_dx)
            ...
            for SNR in snrs:
                for M in snaps:
                    for t in range(args.trials):
                        ...
                        from core.radarpy.metrics import angle_rmse_deg, resolved_indicator
                        ...
                        for alg in algs:
                            try:
                                ...
                                if args.dump_svd:
                                    ...
                                    tag = f"{arr_name}_{alg}_N{args.N}_M{M}_snr{SNR}_trial{t}"
                                    if "Rx_singular" in dbg:
                                        svd_path = f"results/svd/{tag}_Rx_svd.csv"
                                        np.savetxt(svd_path, np.array(dbg["Rx_singular"])[None,:], delimiter=",")
                                    if "Rv_singular" in dbg:
                                        svd_path = f"results/svd/{tag}_Rv_svd.csv"
                                        np.savetxt(svd_path, np.array(dbg["Rv_singular"])[None,:], delimiter=",")
