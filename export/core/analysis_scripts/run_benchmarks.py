...
# Add project root (go up 2 levels from core/analysis_scripts)
...
def run_alg(alg_name, X, positions, d_phys, wavelength, K, scan_deg=(-60, 60, 0.1), use_root=False,
    ...
    if alg_name == "SpatialMUSIC":
        ...
        # dbg_full now includes Rv_singular, Rv_cond, Mv from coarray builder
        dbg = {"Mv": int(dbg_full.get("Mv", 1))}
        # Add SVD diagnostics if present
...
def main():
    ...
    ap.add_argument("--dump-svd", action="store_true",
                    help="Save singular values of Rx/Rv per trial to results/svd/")
    args = ap.parse_args()
