...
class Z1ArrayProcessor(BaseArrayProcessor):
    """
    Weight-Constrained Sparse Array Z1
    Construction (grid units):
      - 2-sparse ULA of (N-1) sensors: { 0, 2, 4, ..., 2(N-2) }
      - One augmented sensor at:       { 2N-1 }
    Key property: w(1) = 0
    Expected one-sided holes: {1, 2N-2}; max positive lag A = 2N-1
    All coarray math is done on integer lags; physical spacing is 'd'.
    """
    ...
    def __init__(self, N: int, d: float = 1.0):
        ...
        # grid-unit positions
        sparse = np.arange(N - 1, dtype=int) * 2          # 0,2,4,...,2(N-2)
        augmented = np.array([2 * N - 1], dtype=int)       # 2N-1
        positions_grid = np.concatenate([sparse, augmented])
        positions_grid.sort()
    ...
    def analyze_coarray(self):
        ...
        # placeholders (one-sided holes computed later)
