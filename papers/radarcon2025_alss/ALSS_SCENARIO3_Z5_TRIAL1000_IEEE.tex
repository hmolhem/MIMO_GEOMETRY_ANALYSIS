...
This paper focuses on a conservative and reproducible evaluation of Adaptive Lag-Selective Shrinkage (ALSS) for the Z5 sparse array. The goal is not to claim universal ALSS optimality across all geometries. Instead, the goal is to verify whether a selected ALSS configuration provides stable RMSE improvement for the Z5 array under Scenario 3 conditions with and without mutual coupling.
...
For an $N$-sensor array with sensor locations $\{n_i\}_{i=1}^{N}$, the narrowband received signal can be written as
\begin{equation}
...
The lag sequence is then used to form a virtual Toeplitz covariance matrix for Coarray MUSIC.
